...
| visit-stage | Specific to a telemedicine visit, the stage of the journey the patient is in. Possible values are ***pre*** which represents the user in the intake process, ***during*** which represents the waiting room or a chat window or ***post***  which is any time after the consult has completed. May not apply to all workflows| No | during
...
| keywords | A list of comma separated keywords like speciality or thereapeutic category associated with the session and used to determine the best content for the opportunity. May also contain other attributes associated with the session like previous diagnosis or codes from a vocabulary like ICD10, NDC, RxNorm, J-code, SNOMED CT, etc. | Yes | |
| gender | Gender the patient was born with or identifies: <br/> ***m*** &rarr; Male, ***f*** &rarr; Female, ***o*** &rarr; Other or ***x*** &rarr; Unknown | Yes | x |
